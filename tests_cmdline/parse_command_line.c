...
void help( void ){
    ...
    printf("WARNING: The text message can't contain special charcaters such as '<', '>' and 'é', 'è', 'â', etc,\n");
	...
	static const struct option long_options[]={
		...
		opt = getopt_long( argc, argv, option_string, long_options, longIndex);
		 ...
		 if ( opt != -1 ){
			 ...
			 //debug
			 //printf("We enter the switch case\n");
		...
		//debug
		//printf("We'are out of the switch case\n");

		/* Save text message if any. */
		 //debug
		// printf("Print optarg[optind] : %d \n", optarg[optind] );

		 //if ( optarg[optind] == '\0' ){
		 //	printf("[!!] WARNING No message has been entered.\n");
		 //}
		...
		/* ATTENTION aussi aux carcatères "<" et ">" pouvant être
		 * interprétés comme des délimiteurs de trames. Ils sont bannis.
		 * */

		 /* Enregistrer le message s'il y en a un.
		  * On accepte les messages vides. */

		 /* On n'accepte des messages de n'importe quelle longueur.
		  * Si le message est trop long pour être inscrit sur le panneau
		  * il sera tronqué ou séparé en plusieurs messages distincts
